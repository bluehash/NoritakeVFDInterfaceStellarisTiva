...
#include "lowLevelDrivers\NoritakeY100\Noritake_VFD_CUY.h"
#include <string.h>
#include "menu.h"


#define SCREEN_BRIGHTNESS       100     // Maximum character brightness
...
#define MENU_SPLIT              (NORITAKE_VFD_COLS>=24? 12: 10) // Width of first column
...
void DetailMenu::draw() {
    ...
    // Print each category
    for (int i = 0; i < MENU_CATEGORIES && i < NORITAKE_VFD_LINES-1; i++) {
        vfd.print("\r\n"); // Move to the next line

        // Center selected line
        if (this->selected < (NORITAKE_VFD_LINES-1)/2)
            cat = this->category + i;
        else {
            int j = i + this->selected - (NORITAKE_VFD_LINES-1)/2;
            if (this->selected > MENU_CATEGORIES - (NORITAKE_VFD_LINES-1))
                j = i + MENU_CATEGORIES - (NORITAKE_VFD_LINES-1);
        ...
        vfd.CUY_setCharset((Charset)1);          // Select Katakana table
        vfd.print("\xe8");              // Print arrow
        vfd.CUY_setCharset((Charset)charset);    // Restore user's character set
    ...
    vfd.CUY_setCursor(MENU_SPLIT,(NORITAKE_VFD_LINES >= 6)? 3: 2);
    leftAlign(cat->caption[1], MENU_SPLIT, NoStyle);
#endif

    // Values
#if NORITAKE_VFD_LINES >= 6
...
#if NORITAKE_VFD_LINES > 2
    vfd.CUY_setCursor(MENU_SPLIT,(NORITAKE_VFD_LINES >= 6)? 4: 3);
...
void Menu_Run( void )
{
    ...
    vfd.CUY_setCharset((Charset)2);
    DetailMenu menu(vfd);

    vfd.CUY_home();
    vfd.CUY_clearScreen();
    vfd.CUY_setFontStyle(TallFont);
    vfd.CUY_home();
    vfd.CUY_setCursor( 9, 2);
    vfd.print("J E E P");
    vfd.CUY_clearScreen();

    menu.category[0] = DetailMenuCategory("PRESSURE",
        "chamber: 01", "PSI",
        "chamber: 02", "PSI");
    ...
    menu.category[0].value[1] = 4560;
        ...
        menu.category[1] = DetailMenuCategory("RPM",
        ...
        menu.category[2] = DetailMenuCategory("MAX FLOW",
            "pipe: 01", "m\xfc/s",
            "pipe: 02", "m\xfc/s");
        ...
        menu.category[3] = DetailMenuCategory(MENU_SPLIT<12? "TTL TIME": "TOTAL TIME",
            "chamber: 01", "hr",
            "chamber: 02", "hr");
        ...
        menu.category[4] = DetailMenuCategory("CURRENT",
            "total AMP", "mA",
            "voltage", "V");
